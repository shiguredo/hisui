...
- ダウンロード中は `.tmp` 一時ファイルに書き込み、完了後にアトミックリネーム
- 自前のリトライ・バックオフは持たない（開発者が手で叩く想定。失敗時は再実行で続きから取れる）

**HF 側でモデルが更新された場合**: CI が SHA256 ミスマッチで落ちて気付くので、最新ファイルを手元で再取得して `sha256sum` で値を取り直し、`TARGETS` dict を更新する PR を出すこと。`expected_sha256` を空文字に戻して 1 回実行すると検証スキップで再取得され、`sha256sum` で値が取れます。
