...
ローカル開発環境で `maturin develop` を実行するためのラッパースクリプト

### 説明

Cargo.toml のバージョンが `-canary.X` 形式を使用している場合、Python/maturin との互換性の問題が発生します。このスクリプトは、**`-canary.X` がある場合のみ**、一時的にバージョンを `-dev.X` 形式に変換してから `maturin develop` を実行し、完了後に元のバージョンに戻します。通常のバージョン（例: `2025.3.0`）の場合は変換せずにそのまま実行します。
