...
- 既存テストの更新: `mp4/hybrid_writer.rs` の映像テスト、`src/decoder/openh264.rs` の `#[cfg(test)]`、`tests/mixer_video_tests.rs` など `VideoFrame.sample_entry` を参照する箇所を型追従させる。
...
- CHANGES.md: 本 issue はリファクタで、利用者から見た録画挙動は不変のため、CLAUDE.md の変更履歴規約に従い `### misc` への記載要否を判断する（未リリース部分の内部変更として記載不要とする判断も可。実装時に確定する）。
