...
5. **fmtp 上限検査追加と共通化**: `select_audio_track` の `size_length == 0` 検査の隣に `size_length > 32` / `index_length > 32` / `index_delta_length > 32` の Err 化を追加し、RFC 3640 §3.3.6 値域外の SDP を session 確立時点で fail-fast するようにした (上限は共有 `BitReader::read_u` の `n > 32` 制約に由来する)。検査ロジックは private helper `validate_aac_fmtp_lengths` に切り出し、`select_audio_track` と後述 PBT helper `depacketize_aac_payload_for_pbt` の双方から呼ぶ単一情報源にした。
