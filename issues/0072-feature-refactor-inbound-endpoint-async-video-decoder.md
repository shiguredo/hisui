...
2. **decoder task lifecycle unit test の実装可能性**: モック禁止で実 `TrackPublisher` を組む手段 (下流 subscriber なしで `send_media` false を強制する経路、 実 subscriber を組んで正常経路を通す経路) が既存ヘルパにあるかを実装段階で確認。 なければ minimum smoke test (spawn → shutdown Ok 経路のみ) に留め、 pipeline closed / panic 経路は pipeline レベルの `cargo test` 統合実行で担保する
...
4. **各 endpoint の module-private helper の共通化タイミング**: 3 endpoint 実装後、 3 箇所の写経がどの程度同型かを diff で確認し、 0073 の未確定論点 4 (共通化 option a/b/c) の判定材料とする。 mp4 reader 版 (`:1528-1643`) は `discard_mode_tx` + `TrackSender` を持ち形が異なるため、 まずは 3 endpoint 3 箇所の同型度を評価する
5. **`handle_input` に相当する `AsyncVideoDecoder` API 追加の要否**: 各 endpoint と mp4 reader で 4 箇所の match `DecoderInput { Media => handle_input_sample_sync(Some(_)), Eos => handle_input_sample_sync(None) }` が同型になる。 冗長と感じる場合は `AsyncVideoDecoder::handle_input(input: DecoderInput)` 相当の helper 追加を 0073 の共通化議論に合流させる
