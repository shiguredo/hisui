...
- `--transcribe-threads <N>`: 1 推論あたりの candle CPU スレッド数 (デフォルト 1)。encoder / decoder とリアルタイム同居させる場合に絞るためのオプション。サブコマンド起動直後 (WhisperPipeline ロードより前) に `std::env::set_var("RAYON_NUM_THREADS", ...)` を叩いて candle の rayon プール初期化前に反映させる (`OnceLock` なので初期化後は変更不可)
