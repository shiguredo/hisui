//! SDP の生成・設定ヘルパー。
//!
//! libwebrtc のコールバックは内部スレッドから呼ばれる非 async コンテキストのため、
//! std::sync::mpsc で結果を受け取る。
...
/// create_offer を実行し、SDP 文字列を返す。
pub fn create_offer_sdp(pc: &PeerConnection) -> crate::Result<String> {
    let mut options = PeerConnectionOfferAnswerOptions::new();
    // データチャネルのみで受信トラックが無い初期オファーなので、受信は無効にする
    options.set_offer_to_receive_audio(0);
    options.set_offer_to_receive_video(0);
    create_sdp(pc, &mut options, true)
}

/// create_answer を実行し、SDP 文字列を返す。
pub fn create_answer_sdp(pc: &PeerConnection) -> crate::Result<String> {
    let mut options = PeerConnectionOfferAnswerOptions::new();
...
/// offer SDP を local description に設定する。
pub fn set_local_offer(pc: &PeerConnection, offer_sdp: &str) -> crate::Result<()> {
    set_local_description(pc, SdpType::Offer, offer_sdp, "offer")
}

/// answer SDP を local description に設定する。
pub fn set_local_answer(pc: &PeerConnection, answer_sdp: &str) -> crate::Result<()> {
    set_local_description(pc, SdpType::Answer, answer_sdp, "answer")
}

/// offer SDP を remote description に設定する。
pub fn set_remote_offer(pc: &PeerConnection, offer_sdp: &str) -> crate::Result<()> {
    set_remote_description(pc, SdpType::Offer, offer_sdp, "offer")
}

/// answer SDP を remote description に設定する。
pub fn set_remote_answer(pc: &PeerConnection, answer_sdp: &str) -> crate::Result<()> {
