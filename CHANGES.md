...
- [CHANGE] 出力 MP4 ファイルが H.265 ストリームを含む場合は hvc1 ボックスを使用する
  ...
  - @sile
- [FIX] inspect が途中で切り詰められた fMP4 (録画プロセスのクラッシュ等で末尾フラグメントが不完全なもの) を demux できずエラーになる問題を修正する
  - trun が宣言するサンプルデータが mdat に収まらないフラグメントを検知した場合は、エラーにせずそこまでに読めたサンプルを返して終了する
  - hybrid mp4 では書き込み途中のクラッシュで不完全な末尾フラグメントが残るのは想定内であり、その手前までの録画内容は有効なため
  - 切り詰めを検知した際は警告ログを出力する
  - @sile
