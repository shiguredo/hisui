...
- [CHANGE] 出力 MP4 ファイルが H.265 ストリームを含む場合は hvc1 ボックスを使用する
  ...
  - @sile
- [FIX] 出力 MP4 の映像トラックが読めなくなる場合がある問題を修正する
  - hybrid_mp4_writer が finalize 時に音声トラック先頭サンプルのサンプルエントリを取りこぼして finalize に失敗し、標準 MP4 への変換が完了しないことがあった
  - writer の入口で受信したサンプルエントリを保持することで、フラグメント先頭サンプルがサンプルエントリを持たなくても finalize できるようにする
  - @sile
