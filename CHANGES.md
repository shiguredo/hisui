...
- [CHANGE] 出力 MP4 ファイルが H.265 ストリームを含む場合は hvc1 ボックスを使用する
  ...
  - @sile
- [FIX] 短時間録画時に StopRecord 後の MP4 から映像トラックが読めなくなる場合がある問題を修正する
  - hybrid_mp4_writer が finalize 時に音声トラック先頭サンプルの sample entry を取りこぼし、finalize に失敗して録画中の空の moov だけが残ることがあった
  - writer の入口で受信した sample entry を保持することで、フラグメント先頭サンプルが sample entry を持たなくても finalize できるようにする
  - @sile
