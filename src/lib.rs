...
pub trait ResultExt {
    ...
    fn or_fail_with<S, F>(self, f: F) -> Result<Self::Output>
    where
        S: Into<String>,
...
impl<T> ResultExt for std::result::Result<T, Error> {
    ...
    where
        ...
        F: FnOnce(Self::Context) -> S,
    {
        self.map_err(|e| Error::new(f(e).into()))
    }
}
...
impl<T> ResultExt for Option<T> {
    ...
    fn or_fail(self) -> Result<Self::Output> {
        self.ok_or_else(|| Error::new("value is missing"))
    }
...
impl ResultExt for bool {
    ...
    fn or_fail(self) -> Result<Self::Output> {
        if self {
            ...
            Err(Error::new("condition is false"))
        }
    }
