//! 汎用ビットリーダー
//!
//! MSB ファーストでバイト列を 1 ビット単位で読み出すユーティリティ。
//! 固定長ビットフィールド (u(n)) と H.264 / H.265 仕様の Exp-Golomb 復号 (ue(v) / se(v)) の
//! 読み出しを提供する。H.264 / H.265 SPS パーサや MPEG4-GENERIC AAC AU header
//! パースで共有する (Exp-Golomb 系メソッドは H.264 / H.265 経路のみで利用)。
