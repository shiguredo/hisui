...
fn make_identify_message() -> String {
    // SoraSource イベント (1 << 20) を subscribe する
    let event_subscriptions = (1 << 20) | ((1 << 10) - 1); // SORA_SOURCE | ALL
    nojson::object(|f| {
        f.member("op", 1)?;
        f.member(
            "d",
...
fn make_request_no_data(request_type: &str) -> (String, String) {
    let request_id = next_request_id();
    let rid = request_id.clone();
    let msg = nojson::object(|f| {
        f.member("op", 6)?;
...
fn make_set_scene_item_transform_request(
    ...
    let sname = scene_name.to_owned();
    make_request("SetSceneItemTransform", move |f| {
        f.member("sceneName", sname.as_str())?;
...
fn make_set_record_directory_request(directory: &str) -> (String, String) {
    let dir = directory.to_owned();
    make_request("SetRecordDirectory", move |f| {
        f.member("recordDirectory", dir.as_str())
    })
}

fn make_start_record_request() -> (String, String) {
    make_request_no_data("StartRecord")
}
...
fn parse_request_response(text: &str) -> Option<(String, Result<String, String>)> {
    ...
    if result {
        // responseData があればその JSON 文字列を返す
        let response_data = d
...
async fn update_grid_layout(
    ...
    for (i, entry) in state.video_tracks.iter().enumerate() {
        let col = i % cols;
        let row = i / cols;
...
fn main() -> noargs::Result<()> {
    ...
    args.metadata_mut().app_name = "sora_source";
...
async fn run(
    ...
    tracing::info!(
        "イベント待受開始（{duration} 秒間）。配信者を追加してください。Ctrl+C で中断。"
    );
    ...
    loop {
        ...
        tokio::select! {
            ...
            _ = tokio::time::sleep_until(deadline) => {
                tracing::info!("録画時間 {duration} 秒経過");
                break;
            }
