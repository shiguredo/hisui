...
レイアウトJSON の `video_sources` で指定されたパスがワイルドカードを含む場合は、まずそれが解決されます:
...
メタデータファイルの `start_time_offset` と `stop_time_offset` の値をそのまま使用します。

### 分割録画の場合

全ての分割ファイルの時間範囲を統合し、最も早い開始時刻と最も遅い終了時刻を使用します。
