# リージョンについて {#layout-region}

## リージョンの概要 {#layout-region-overview}
...
## 基本概念 {#layout-region-basics}

### リージョン {#layout-region-concept-region}
...
### グリッド {#layout-region-concept-grid}

リージョン内でのセルの配置パターンです。
行数と列数で構成され、映像ソースの数や `max_rows`、`max_columns` の設定に基づいて自動的に決定されます。

### 映像ソース {#layout-region-concept-source}
...
## リージョン処理の全体の流れ {#layout-region-process-overview}
...
## 映像ソース一覧と表示時間範囲の決定方法 {#layout-region-sources}

リージョンで使われる映像ソース一覧の決定は、以下の手順で行われます。

### 1. `video_sources` のワイルドカード解決 {#layout-region-sources-step1}
...
- 対応するメディアファイルが存在しないメタデータファイルは、ワイルドカード展開の結果から除外されます
  ...
  - なお、ワイルドカードではなく直接指定したメタデータファイルに対応するメディアファイルが存在しない場合はエラーとなります

### 2. `video_sources_excluded` のワイルドカード解決と除外処理 {#layout-region-sources-step2}
...
### 3. 分割録画の考慮 {#layout-region-sources-step3}
...
### 4. 映像ソースの表示開始時刻と終了時刻の決定 {#layout-region-sources-step4}
...
## グリッドの構成（行列サイズ）の決定方法 {#layout-region-grid}

各リージョン内でのグリッドの行数と列数は、映像ソースの数と制約設定に基づいて自動的に決定されます。
決定処理は以下の手順で行われます。

### 1. リージョンで必要なセル数を算出 {#layout-region-grid-step1}
...
### 2. グリッドサイズの決定 {#layout-region-grid-step2}

グリッドサイズの決定は、`max_rows` および`max_columns` による制約の指定状況に応じて以下の方針で行われます。

#### 制約が全く指定されていない場合 {#layout-region-grid-unconstrained}

`max_rows` と `max_columns` の両方が未指定の場合、以下の方針で行列数を決定します。
...
#### 制約が一方のみ指定されている場合 {#layout-region-grid-single-constraint}

制約が一方のみ指定されている場合、指定された制約値を基準として処理されます。

**`max_columns` のみが指定されている場合**

- 列数を必要なソース数と `max_columns` の小さい方に設定
- 行数を必要なセル数に基づいて計算（`必要なセル数 ÷ 列数` を切り上げ）

**例**: 5つのソースで `max_columns = 4` が指定されている場合
...
│Src1 │Src2 │Src3 │Src4 │
├─────┼─────┼─────┼─────┤
│Src5 │Empty│Empty│Empty│
└─────┴─────┴─────┴─────┘
```

**`max_rows` のみが指定されている場合**

- 行数を必要なソース数と `max_rows` の小さい方に設定  
- 列数を必要なセル数に基づいて計算（`必要なセル数 ÷ 行数` を切り上げ）

**例**: 5つのソースで `max_rows = 4` が指定されている場合
...
#### 両方の制約が指定されている場合 {#layout-region-grid-both-constraints}

`max_rows` と `max_columns` の両方が指定されている場合は、より小さい制約値を持つ軸を優先して処理されます。

- `max_columns <= max_rows` の場合
  - 「`max_columns` のみが指定されている場合」と同じ方法でグリッドサイズを決定する
  - ただし、行のサイズは最大でも `max_rows` とする

- `max_columns > max_rows` の場合
  - 「`max_rows` のみが指定されている場合」と同じ方法でグリッドサイズを決定する
  - ただし、列のサイズは最大でも `max_columns` とする

## 各映像ソースのセルへの割り当て {#layout-region-assignment}
...
### 基本的な割り当て方針 {#layout-region-assignment-basic}
...
### 十分な空きセルがある場合の動作例 {#layout-region-assignment-ample}
...
### `reuse` 設定による動作の違い {#layout-region-assignment-reuse}
...
#### `reuse: "none"` の場合 {#layout-region-assignment-reuse-none}
...
#### `reuse: "show_oldest"`（デフォルト）の場合 {#layout-region-assignment-reuse-oldest}
...
#### `reuse: "show_newest"` の場合 {#layout-region-assignment-reuse-newest}
...
## 解像度の決定方法 {#layout-region-resolution}
...
### 全体解像度の決定方法 {#layout-region-resolution-overall}

全体解像度は、合成後の映像全体のサイズを決定します。
以下の優先順位で決定されます。

#### 1. `resolution` フィールドによる明示的指定 {#layout-region-resolution-overall-step1}
...
#### 2. リージョンの配置からの自動計算 {#layout-region-resolution-overall-step2}
...
### リージョン解像度の決定方法 {#layout-region-resolution-region}

各リージョンのサイズは、以下の優先順位で決定されます。

#### 1. `width` / `height` による明示的指定 {#layout-region-resolution-region-step1}
...
#### 2. `cell_width` / `cell_height` からの計算 {#layout-region-resolution-region-step2}
...
この例では、内側の枠線（2 ピクセル × 1 本ずつ）と外側の枠線（2 ピクセル × 2 辺ずつ）が加算され、
最終的なリージョンサイズは `646x484` ピクセルとなります。

枠線の扱いについては、セル解像度の決定方法のセクションで詳しく説明しています。
...
#### 3. 全体解像度からの自動計算 {#layout-region-resolution-region-step3}
...
### セル解像度の決定方法 {#layout-region-resolution-cell}
...
- **内側の枠線**:
  - セル間に挿入される枠線で、幅は `border_pixels` で指定された値となります（デフォルトは 2 ピクセル）
- **外側の枠線**:
  - リージョンの外周に挿入される枠線で、可能な場合は `border_pixels` で指定された値、そうでなければ残りの領域に合わせて調整されます
  - ただし、リージョンが全体解像度と同じサイズの場合、外側の枠線は挿入されません

`border_pixels` はリージョンごとに設定可能で、レイアウト JSON の `video_layout.$REGION_NAME.border_pixels` で指定します。
省略された場合のデフォルト値は 2 ピクセルです。
...
2. **外側の枠線**: リージョンサイズ (640×480) が全体解像度 (1920×1080) より小さいため、上下左右に 4 ピクセルずつ
3. **内側の枠線**: セル間に 4 ピクセルずつ（水平方向に 1 本、垂直方向に 1 本）
4. **セルサイズの計算**:
   - セル幅: `(640 - 8 - 4) / 2 = 314` ピクセル
   - セル高: `(480 - 8 - 4) / 2 = 234` ピクセル

**枠線を無効化する場合**:

枠線を完全に無効化したい場合は、`border_pixels` に `0` を指定します
...
この場合、セル間や外周に枠線は挿入されず、リージョン全体がセルで埋め尽くされます。

## 映像の合成処理 {#layout-region-composition}

ここまでで各リージョンの構成や映像ソースのセルへの割り当てが決定しました。
本セクションでは、それらの情報を使って、どのように合成処理が行われるのかを概説します。

### リージョンの描画 {#layout-region-composition-region}
...
### 映像ソースのセルへの描画 {#layout-region-composition-cell}

各セルに割り当てられた映像ソースは、以下の手順で描画されます：

#### 開始時刻と終了時刻による描画制御 {#layout-region-composition-timing}
...
#### 同じセルに割り当てられた場合の優先順序 {#layout-region-composition-priority}
...
#### ソースのリサイズ（アスペクト比の維持） {#layout-region-composition-resize}
...
#### 描画対象のソースが存在しないセルの扱い（黒塗り） {#layout-region-composition-empty}
...
### `trim` の扱い {#layout-region-composition-trim}

トリムは、配信者が存在しない期間を合成結果から自動的に除去する機能です。

レイアウト JSON で `trim: true` を指定することで有効になります（省略時は有効扱い）
