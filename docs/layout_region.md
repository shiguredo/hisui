...
- `none`: セルを再利用しない（空きセルがない時に開始された映像ソースは表示対象から除外）
- `show_oldest`（デフォルト）: セルを再利用し、競合時は開始時刻が早いソースを優先
...
利用可能なセルがない時に開始された映像ソースは、セルへの割り当てが行われません。
...
時刻 5:
┌──────┬──────┐
│  C   │  A   │  ← ソース B が開始するが空きセルがないため割り当てられない
└──────┴──────┘
...
この場合のリージョンのサイズは最大でおよそ `(320*2)x(240*2) = 640x480` となります。

なお、実際のリージョンサイズにはセルの周りに挿入される枠線の分が加算されるため、最大値は上の値よりも大きくなります。
...
- **`reuse: "show_oldest"`**: 優先度の値が小さい（開始時刻が早い）ソースが優先的に表示される
- **`reuse: "show_newest"`**: 優先度の値が大きい（開始時刻が遅い）ソースが優先的に表示される

優先度の低いソースは、同じセルに割り当てられた優先度の高いソースが表示されている間は描画されません。
...
- **優先度の低いソース**: 同じセルに複数のソースが割り当てられているが、優先度が低いため表示されないソース
...
- **判断基準**: ソースの映像・音声の内容とは関係なく、**ソースが該当期間に存在するかどうか**のみで判断されます
- **適用範囲**: 映像だけでなく音声も含めて処理されます
- **除去対象**: 音声ないし映像ソースが全く存在しない時間帯が、合成結果に含まれなくなります
...
- **0-2秒**: ソース C のみ存在 → トリムされない
- **2-5秒**: ソース A と C が存在 → トリムされない  
- **5-10秒**: ソース A のみ存在 → トリムされない
- **10-15秒**: ソースが存在しない → **トリムされる**（合成結果から除去）
- **15-20秒**: ソース B と C が存在 → トリムされない
- **20-25秒**: ソース B と C が存在 → トリムされない  
- **25-30秒**: ソース C のみ存在 → トリムされない

結果として、10-15秒の期間が合成結果から除去され、実際の出力時間は25秒（30秒 - 5秒）となります。
