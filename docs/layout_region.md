...
各リージョンの処理は、以下の流れで実行されます：

1. **映像ソース一覧の決定**
  - `video_sources`のワイルドカード解決、`video_sources_excluded`の除外処理、同一`connection_id`を持つ分割録画ファイルの統合
2. **映像ソースの時間範囲確定**
  - 各ソースの表示開始時刻・終了時刻の決定（分割録画の場合は連続する時間範囲として統合）
3. **最大同時ソース数の算出**
  - 時間的重複とセル除外（`cells_excluded`）を考慮した同時表示が必要な最大ソース数の計算
4. **グリッド構成の決定**
  - `max_rows`、`max_columns`制約下での最適なグリッド行列数の決定
5. **解像度の段階的確定**
  - セル解像度指定からのリージョン解像度逆算、全体解像度の確定、最終的なセル解像度の計算（枠線考慮）
6. **ソースのセルへの割り当て**
  - `reuse`設定に基づく割り当てアルゴリズムの実行、時間的重複時の優先度決定
7. **枠線・位置の最終調整**
  - 内側枠線（セル間境界）と外側枠線の計算、リージョンの絶対位置確定
8. **トリム範囲の決定**
  - `trim: true`指定時の全ソース非存在時間帯の特定と除去範囲決定
9. **合成処理での利用**
  - Z座標順でのリージョン描画、時間経過に応じたセル内映像ソース切り替え、フレームサイズの動的調整
...
まず、そのリージョンで同時に表示が必要となる映像ソースの最大数を、以下のようにして算出します。

[WIP]
1. リージョンで使用する映像ソース一覧を、以下の手順で決定する
   1. `video_sources` から `video_sources_excluded` を除外
