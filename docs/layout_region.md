...
`reuse: "show_oldest"` を指定すると、セルを再利用し、競合が発生した場合には開始時刻が早い映像ソースが優先されます。
...
時刻 5:
┌──────┬──────┐
│  C   │  A   │  ← ソース B が開始するが、ソース A の方が早く開始したため A が優先
└──────┴──────┘
...
時刻 10:
┌──────┬──────┐
│  B   │      │  ← ソース A が終了し、ソース B がセル 0 に再配置される
└──────┴──────┘

時刻 14:
┌──────┬──────┐
│      │      │  ← ソース B が終了してセル 0 が空きに
└──────┴──────┘
```

この例では、時刻 5-8 の期間中にソース B が開始しますが、既に利用中のソース A の方が開始時刻が早いため、ソース A が表示され続けます。ソース A が終了した後に、ソース B が表示されます。

#### `reuse: "show_newest"` の場合

`reuse: "show_newest"` を指定すると、セルを再利用し、競合が発生した場合には開始時刻が遅い映像ソースが優先されます。
...
時刻 5:
┌──────┬──────┐
│  C   │  B   │  ← ソース B が開始し、ソース A より後に開始したため B が優先される
└──────┴──────┘

時刻 8:
┌──────┬──────┐
│      │  B   │  ← ソース C が終了してセル 0 が空きに
└──────┴──────┘

時刻 14:
┌──────┬──────┐
│      │      │  ← ソース B が終了してセル 1 が空きに
└──────┴──────┘
```

この例では、時刻 5 でソース B が開始すると、既に表示中のソース A よりも開始時刻が遅いため、ソース B がソース A を押し出してセル 1 に表示されます。結果として、ソース A は時刻 5 以降は表示されなくなります。

**重要なポイント:**

- `show_oldest` では、既に表示中のソースが新しく開始されたソースよりも優先される
- `show_newest` では、新しく開始されたソースが既に表示中のソースを押し出す
- どちらの場合でも、セルが空いている場合は新しいソースが空きセルに割り当てられる
