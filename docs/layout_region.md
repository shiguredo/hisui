...
4 つのセル（2×2）に 3 つの映像ソースがある場合の基本的な動作：
...
時刻10: ┌──────┬──────┐
        │ 空き │ A(1) │
        ├──────┼──────┤
        │ B(2) │ 空き │  ← ソース B が開始して最初の空きセル(2) に割り当て
        └──────┴──────┘

時刻15: ┌──────┬──────┐
        │ 空き │ 空き │  ← ソース A が終了してセル 1 が空きに
        ├──────┼──────┤
        │ B(2) │ 空き │
        └──────┴──────┘
...
時刻18: ┌──────┬──────┐
        │ 空き │ 空き │
        ├──────┼──────┤
        │ 空き │ 空き │  ← ソース B が終了して全セルが空きに
        └──────┴──────┘
```

この例では、セルが十分にあるため、どのソースも競合することなく表示されています。
セル内の数字（0, 1, 2）はセルのインデックスを表しています。
