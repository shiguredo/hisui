...
グリッドの構成が決定された後、各映像ソースを具体的なセルに割り当てる処理が行われます。
この処理では、映像ソースの表示時間範囲とセルの再利用設定（`reuse`）を考慮して、最適な配置を決定します。

### 基本的な割り当て方針

映像ソースのセルへの割り当ては、以下の手順で行われます：

1. **セルの初期状態設定**: 利用可能なセルを「空き」状態に、除外されたセル（`cells_excluded`）を「除外」状態に設定
2. **映像ソースの時系列順ソート**: 開始時刻が早い順に処理するため、全映像ソースを時系列順にソート
3. **順次割り当て**: 各映像ソースに対して、`reuse` 設定に従ってセルを割り当て
