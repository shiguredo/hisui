...
## 最適化（エンコードパラメーターの探索・調整） のモチベーション {#command-tune-motivation}
...
## エンコードパラメーター調整の流れ {#command-tune-flow}

`hisui tune` を利用してエンコードパラメーターの調整を行う際の流れは、典型的には次のようになります。

1. ベースとするレイアウトファイルを決定します
   - 既に運用で使っているものがあるなら、それをベースにするのがいいです
   - そういったものがない場合には、Hisui リポジトリの `layout-examples/tune-*.json` ファイルが参考になります
2. 上のレイアウトファイルの中で、調整対象となるパラメーターを決定します
   - レイアウトファイルの中で値が null の項目が調整対象となります（詳細は後述）
3. 探索で使用する録画ファイルを用意します
   - `hisui tune` は「パラメーターを変えつつ小規模な合成を行う」というプロセスを繰り返し実施します
   - そのため、その探索用の合成処理で使用する録画ファイルが必要となります
4. `hisui tune` を実行します
   - `hisui tune` は実際の運用環境（OS やマシン）に合わせた環境で実行するのが重要です
   - `hisui tune` を実行すると、`hisui compose` でそのまま利用可能な、調整後のレイアウトファイル群が生成されます
5. `hisui tune` によって見つかったレイアウトファイル群の中から、よさそうな候補をいくつか選択します
   - 詳しくは後述しますが、`hisui tune` では「エンコード時間（短いほどいい）」と「エンコード品質（高いほどいい）」という 2 つの指標で探索を行います
   - この 2 つの指標は一般にトレードオフ関係にあるため「単一なベストな結果」ではなく、時間か品質のどちらかの面で優れている、複数の結果が得られることになります
   - その中からどれを選択すべきかは実際のユースケース次第となります
6. `hisui compose` で、候補レイアウトファイルを使って実際に合成を行い、最終的に採用するレイアウトファイルを決定します
   - `hisui tune` では時間の節約のために部分的な合成しか行わず、また品質の評価が人間の感覚と一致しているとは限りません
   - そのため、実際に自分でいくつか合成してみて結果を確認することが重要です
   - また探索によって、必ずしもベースのレイアウトファイルよりもいいものが見つかる保証はないため、この段階で「もとのファイルをそのまま使用する」という判断となる可能性もあります
7. 必要に応じて、最終的なレイアウトファイルの内容を手で微調整します
   - もし対象のエンコードパラメーターについて詳しい人がいる場合には、手動調整を加えることで、さらなる改善が期待できます

## 依存パッケージ {#command-tune-dependencies}
...
### Ubuntu で利用する場合 {#command-tune-deps-ubuntu}

Ubuntu では依存パッケージをビルドする必要があります。

#### vmaf のビルド {#command-tune-deps-vmaf-build}
...
#### optuna のインストール {#command-tune-deps-optuna}
...
## Optuna による最適化の概要 {#command-tune-optuna-overview}

### 最適化の流れ {#command-tune-optuna-flow}
...
### 最適化メトリクス {#command-tune-optuna-metrics}
...
## 実行例 {#command-tune-examples}

### デフォルト設定での実行 {#command-tune-example-default}
...
最適解集合の表示には、次のように、
見つかったパラメーターセットを使った合成を行うコマンドの例も含まれているので、簡単に合成を試すことができます。
...
## 探索用のレイアウトファイル {#command-tune-layout-files}
...
## 探索空間定義ファイル {#command-tune-search-space}
...
## Tips {#command-tune-tips}

### 探索に使用するレイアウトファイルの作成方法 {#command-tune-tip-layout}
...
### 探索に使用する探索空間定義ファイルはどうすべきか {#command-tune-tip-search-space}
...
### 探索対象から除外した方がいいパラメーター {#command-tune-tip-exclude-params}
...
### 探索結果から最終的に使用するパラメーターセット（レイアウト JSON）をどうやって選択するか {#command-tune-tip-select-results}
...
### 探索の実行環境や使用する録画ファイルについて {#command-tune-tip-environment}
...
### 実際のエンコードビットレートや合成処理時間などを確認する方法 {#command-tune-tip-check-metrics}
...
### 以前の探索の続きから再開したい場合 {#command-tune-tip-resume}
...
### 探索空間やレイアウトファイルを変更して探索を行いたい場合 {#command-tune-tip-change-settings}
...
### トライアル回数をどうすべきか {#command-tune-tip-trial-count}
...
### トライアルが失敗した場合にどうすべきか {#command-tune-tip-trial-failure}
...
### 各トライアルの実行時間を短くしたい {#command-tune-tip-reduce-time}
...
### 実行済み探索の最適解集合のみを確認したい {#command-tune-tip-review-results}
