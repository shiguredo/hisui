...
このコマンドは、多目的最適化アルゴリズム NSGA-II（Hisui に内蔵）を使用して、指定されたレイアウトファイル内のエンコードパラメーターを自動的に調整し、**合成実行時間の最小化**と **映像品質（VMAF スコア）の最大化**という 2 つの目的を両立する最適なパラメーターセットを探索します。
...
このコマンドの多目的最適化は Hisui に内蔵された NSGA-II 実装で行われるため、最適化のための外部パッケージのインストールは不要です。
...
なお Hisui の `--trial-count` は外部 Optuna を使っていた頃とは意味が異なり、「追加の試行回数」ではなく「既存の履歴を含めた合計の到達回数」である点に注意してください。
