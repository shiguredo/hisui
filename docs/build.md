...
```bash
sudo apt-get install -y meson ninja-build nasm yasm build-essential autoconf automake libtool pkg-config yasm cmake libx11-dev libpulse-dev clang
```
...
Commands:
  ...
  tune        NSGA-II を用いた映像エンコードパラメーターの調整を行います
