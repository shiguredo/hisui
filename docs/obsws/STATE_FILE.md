...
- 永続化対象: output 設定（`stream` / `record` / `rtmp_outbound` / `sora` / `hls` / `mpeg_dash`）、SoraSubscriber 設定、scene 構成、input 定義、scene item 配置
...
SoraSubscriber の設定一覧。配列形式で、各要素は以下のフィールドを持つ。

| フィールド | 型 | 必須 | 説明 |
|-----------|-----|------|------|
| `subscriberName` | String | 必須 | subscriber 名（一意識別子） |
| `signalingUrls` | String[] | 省略可 | シグナリング URL リスト |
| `channelId` | String | 省略可 | チャンネル ID |
| `clientId` | String | 省略可 | クライアント ID |
| `bundleId` | String | 省略可 | バンドル ID |
