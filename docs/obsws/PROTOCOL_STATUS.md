...
- [x] `GetInputKindList`: 入力種別一覧を取得する
  - NOTE: hisui の inputKind 一覧は OBS と異なる。共通は `image_source` / `color_source` のみ。hisui 固有の種別は「hisui 独自拡張」セクション参照
...
- [x] `GetInputDefaultSettings`: 入力の既定設定を取得する
  - NOTE: 現在は `image_source` / `color_source` / `video_capture_device` / `audio_capture_device` / `mp4_file_source` / `rtmp_inbound` / `srt_inbound` / `rtsp_subscriber` / `webrtc_source` の既定設定を返す
  - NOTE: hisui の `image_source` デフォルト設定は `{}` を返す。OBS は `linear_alpha` / `unload` 等のプロパティを含む
- [x] `GetInputPropertiesListPropertyItems`: 入力プロパティのリスト型項目を取得する
  - NOTE: `video_capture_device` の `device_id` / `formats` / `pixel_format` / `fps` プロパティの列挙に対応する
  - NOTE: `audio_capture_device` の `device_id` / `sample_rate` / `channels` プロパティの列挙に対応する（入力デバイスのみを対象にする）
  - NOTE: 上記以外の入力に対してはエラーを返す
...
- [x] `SetInputSettings`: 入力設定を更新する
  ...
  - NOTE: 設定更新により source の起動・停止が起こりうる
...
- [x] `GetOutputList`: 出力一覧を取得する
  - NOTE: `stream` / `record` / `rtmp_outbound` / `sora` / `hls` / `mpeg_dash` / `player` の 7 出力を返す（`player` は `player` feature 有効時のみ）
- [x] `GetOutputStatus`: 出力状態を取得する
  - NOTE: `outputName` は `stream` / `record` / `rtmp_outbound` / `sora` / `hls` / `mpeg_dash` / `player` を受理する
...
- [x] `GetMediaInputStatus`: メディア入力状態を取得する
  - NOTE: 現時点では `mp4_file_source` のみ対応する。それ以外の `inputKind` にはエラーを返す
  - NOTE: `mediaState` は `OBS_MEDIA_STATE_NONE` / `OBS_MEDIA_STATE_PLAYING` / `OBS_MEDIA_STATE_PAUSED` / `OBS_MEDIA_STATE_STOPPED` / `OBS_MEDIA_STATE_ENDED` を返す
  - NOTE: `mediaDuration` と `mediaCursor` はミリ秒単位で返す
- [x] `SetMediaInputCursor`: メディア入力カーソル位置を設定する
  - NOTE: 現時点では `mp4_file_source` のみ対応する
  - NOTE: `mediaCursor` はミリ秒単位で指定し、`0..=mediaDuration` に clamp する
  - NOTE: シークは再生状態を変更しない。Paused / Stopped / Ended 中のシークは次の Play で反映される
  - NOTE: 非キーフレーム位置を指定した場合、内部では直前のキーフレームからデコーダーの warm-up を行い、要求位置未満のフレームは publish しない
- [x] `OffsetMediaInputCursor`: メディア入力カーソル位置を相対移動する
  - NOTE: 現時点では `mp4_file_source` のみ対応する
  - NOTE: `mediaCursorOffset` はミリ秒単位で指定する。現在位置に加算した結果を `0..=mediaDuration` に clamp する
- [x] `TriggerMediaInputAction`: メディア入力アクションを実行する
  - NOTE: 現時点では `mp4_file_source` のみ対応する
  - NOTE: 対応アクション: `OBS_WEBSOCKET_MEDIA_INPUT_ACTION_PLAY` / `_PAUSE` / `_STOP` / `_RESTART`
  - NOTE: 成功時は MediaInputs 購読中セッションへ `MediaInputActionTriggered` を配信する
...
#### `webrtc_source` の利用フロー

`webrtc_source` は、WebRTC 確立後に client 側で追加した upstream video track を hisui の input に接続するための仕組みである。

基本フローは以下の通り。

1. client 側で video track を `RTCPeerConnection` に追加する
2. track を追加した client 側が renegotiation offer を送る
3. server 側が answer を返す
4. renegotiation 完了後に `HisuiListWebRtcVideoTracks` で現在の upstream video track 一覧を取得する
5. `CreateInput` で `webrtc_source` input を作成する
6. `HisuiAttachWebRtcVideoTrack` で `trackId` を `webrtc_source` input に接続する

- NOTE: `HisuiAttachWebRtcVideoTrack` / `HisuiDetachWebRtcVideoTrack` 自体は SDP 更新を行わない
- NOTE: WebRTC 上で track を追加・削除した側が offerer になる

#### `HisuiListWebRtcVideoTracks`

`obsws` DataChannel 経由で、現在の bootstrap セッションで client から送信中の video track 一覧を取得する。
...
- NOTE: track が 0 本でも成功で空配列を返す
- NOTE: 対象は video のみ
- NOTE: 単発 Request（op=6）でのみ使用可能。RequestBatch（op=8）には対応していない

#### `HisuiAttachWebRtcVideoTrack`

`obsws` DataChannel 経由で、bootstrap セッション上の client 送信 video track を既存の `webrtc_source` input に接続する。
...
- NOTE: `webrtc_source` 以外の input kind への attach はエラー
- NOTE: 存在しない trackId、非 video track はエラー
- NOTE: 別 input に attach 済みの track はエラー
- NOTE: 既にその input に別 trackId が attach 済みなら差し替え可能。旧 track 側の接続は解除される
- NOTE: `trackId` は client 側の renegotiation 完了後に `HisuiListWebRtcVideoTracks` で取得する
- NOTE: WebRTC renegotiation 自体は別で、attach は既存 track の論理接続だけを行う
- NOTE: 単発 Request（op=6）でのみ使用可能。RequestBatch（op=8）には対応していない

#### `HisuiDetachWebRtcVideoTrack`

`obsws` DataChannel 経由で、`webrtc_source` input と上り video track の接続を外す。
...
bootstrap セッションの上り video track を受けるための input kind。
`CreateInput` で作成し、`HisuiAttachWebRtcVideoTrack` で WebRTC video track を接続する。
...
| `HisuiAttachSoraSourceTrack` | トラックを `sora_source` 入力に紐付け | `inputName`, `connectionId`, `trackKind` (`"video"` or `"audio"`) |
| `HisuiDetachSoraSourceTrack` | トラックを `sora_source` 入力から解除 | `inputName`, `trackKind` |
...
- 自律的な source processor を持たない（`HisuiAttachSoraSourceTrack` でトラックを紐付けてフレーム転送を開始する）
- 接続の切断・再接続をまたいでシーン上の配置を保持する

**フロー:**

1. `CreateInput` で `sora_source` 入力を作成（シーンに配置）
2. `HisuiStartSoraSubscriber` で subscriber を作成して RecvOnly 接続を開始
3. `SoraSourceTrackPublished` イベントでトラック到着を検知
4. `HisuiAttachSoraSourceTrack` でトラックを入力に紐付け
5. 接続切断時: `SoraSourceTrackUnpublished` で自動 detach（入力自体は残る）
6. 再接続時: 新しい `connectionId` で再 attach → mixer 配置はそのまま
7. `HisuiStopSoraSubscriber` で接続を停止し subscriber を削除
...
Program 出力を SDL3 ウィンドウにリアルタイム表示する。`player` feature が有効な場合のみ利用可能。

- `outputKind`: `player_output`
...
合成映像にテキストをオーバーレイ描画する。サーバ起動時に `--font-search-root` と `--default-font` の両方を指定した場合のみ利用可能。`VideoRealtimeMixer` の最終 InputTrack として z = `i64::MAX` で合成され、出力は `mp4_output` / `hls_output` 等にもそのまま反映される。
