...
- [x] `GetInputDefaultSettings`: 入力の既定設定を取得する
  - NOTE: 現在は `image_source` / `video_capture_device` / `audio_capture_device` / `mp4_file_source` / `rtmp_inbound` / `srt_inbound` / `rtsp_subscriber` / `webrtc_source` の既定設定を返す
...
- [x] `StartStream`: 配信を開始する
  ...
  - NOTE: 入力対応は `image_source` / `video_capture_device` / `audio_capture_device` / `mp4_file_source` / `rtmp_inbound` / `srt_inbound` / `rtsp_subscriber` / `webrtc_source`
