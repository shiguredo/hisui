...
4. renegotiation 完了後に `ListWebRtcVideoTracks` で現在の upstream video track 一覧を取得する
5. `CreateInput` で `webrtc_source` input を作成する
6. `AttachWebRtcVideoTrack` で `trackId` を `webrtc_source` input に接続する

- NOTE: `AttachWebRtcVideoTrack` / `DetachWebRtcVideoTrack` 自体は SDP 更新を行わない
- NOTE: WebRTC 上で track を追加・削除した側が offerer になる

#### `ListWebRtcVideoTracks`
...
- NOTE: `trackId` は client 側の renegotiation 完了後に `ListWebRtcVideoTracks` で取得する
- NOTE: WebRTC renegotiation 自体は別で、attach は既存 track の論理接続だけを行う
- NOTE: 単発 Request（op=6）でのみ使用可能。RequestBatch（op=8）には対応していない

#### `DetachWebRtcVideoTrack`
...
`CreateInput` で作成し、`AttachWebRtcVideoTrack` で WebRTC video track を接続する。
...
- NOTE: `trackId` は読み取り専用で、`AttachWebRtcVideoTrack` / `DetachWebRtcVideoTrack` / bootstrap セッション切断時に更新される
