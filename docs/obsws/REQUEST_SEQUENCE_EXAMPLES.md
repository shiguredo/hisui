...
hisui 対応状況: 部分対応（ `StartStream` は `image_source` / `video_capture_device` / `audio_capture_device` / `mp4_file_source` + `rtmp_custom` 前提 ）
...
目的: 録画の基本制御を行う
hisui 対応状況: 部分対応（ `GetRecordStatus` / `ToggleRecord` / `StartRecord` / `StopRecord` は対応済み。入力種別と構成に制約あり ）
...
NOTE: `GetInputDefaultSettings` は現時点で `image_source` / `video_capture_device` / `audio_capture_device` / `mp4_file_source` を返す
...
1. `C -> S` CreateSoraSubscriber（ `subscriberName = "sub1"`, `signalingUrls = ["wss://sora.example.com/signaling"]`, `channelId = "room1"` ）
2. `S -> C` RequestResponse（ success ）
3. `C -> S` CreateInput（ `sceneName = "Scene"`, `inputName = "camera_user_a"`, `inputKind = "sora_source"` ）
4. `S -> C` RequestResponse（ success ）
5. `C -> S` StartSoraSubscriber（ `subscriberName = "sub1"` ）
6. `S -> C` RequestResponse（ success ）
7. `S -> C` Event（ `eventType = "SoraSubscriberNotify"`, `notify = { "event_type": "connection.created", "connection_id": "conn1", "client_id": "user_a", ... }` ）
8. `S -> C` Event（ `eventType = "SoraSourceTrackPublished"`, `subscriberName = "sub1"`, `connectionId = "conn1"`, `clientId = "user_a"`, `trackKind = "video"`, `trackId = "track1"` ）
9. `C -> S` AttachSoraSourceTrack（ `inputName = "camera_user_a"`, `connectionId = "conn1"`, `trackKind = "video"` ）
10. `S -> C` RequestResponse（ success ）

**再接続フロー**: user_a が切断後に再接続した場合

11. `S -> C` Event（ `eventType = "SoraSourceTrackUnpublished"`, `connectionId = "conn1"`, `trackKind = "video"` ）
    - 自動 detach が発生（camera_user_a 入力は空になるが、シーン上の配置は保持される）
12. `S -> C` Event（ `eventType = "SoraSourceTrackPublished"`, `connectionId = "conn2"`, `clientId = "user_a"`, `trackKind = "video"` ）
13. `C -> S` AttachSoraSourceTrack（ `inputName = "camera_user_a"`, `connectionId = "conn2"`, `trackKind = "video"` ）
14. `S -> C` RequestResponse（ success ）
