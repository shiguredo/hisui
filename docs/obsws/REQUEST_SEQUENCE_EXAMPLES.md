...
hisui 対応状況: 部分対応（ `StartStream` は `image_source` / `video_capture_device` / `mp4_file_source` + `rtmp_custom` 前提 ）
...
NOTE: `GetInputDefaultSettings` は現時点で `image_source` / `video_capture_device` / `mp4_file_source` を返す
