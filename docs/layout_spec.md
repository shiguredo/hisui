...
指定可能な値の範囲は 1 以上の整数です。デフォルト値は未指定（制限なし）で、この場合は映像ソース数に基づいて自動的に列数が決定されます。
...
指定可能な値の範囲は 1 以上の整数です。デフォルト値は未指定（制限なし）で、この場合は映像ソース数に基づいて自動的に行数が決定されます。
...
上記の例では、映像ソースが何個あっても、メインリージョンのグリッドは最大2行までに制限されます。
...
- `"none"`: セルを再利用しません。利用可能なセルがなくなった場合、それ以降の映像ソースは表示されません。
