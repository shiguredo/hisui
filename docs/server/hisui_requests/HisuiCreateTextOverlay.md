...
| `z` | integer | - | テキストオーバーレイ間の z-order (i32 範囲)。省略時は宣言順 (現在登録されている最大 z + 1 が自動割り当てされ、後勝ち) |
...
- 必須フィールド (`textOverlayName` / `text` / `x` / `y` / `fontSize`) の欠落・空文字列・型不一致: `REQUEST_STATUS_MISSING_REQUEST_FIELD` (300)
