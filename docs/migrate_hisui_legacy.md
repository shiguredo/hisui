...
Linux 環境で、H.264 のエンコードおよびデコードを行うためには OpenH264 ないし NVIDIA Video Codec が必須となります。
...
新 Hisui では、常にこの方式を採用し、またその際に（レガシー Hisui と異なり）一時ファイルを利用しなくなりました。
...
基本的には、レガシー Hisui でも新 Hisui でも、映像ソースはその開始時刻の順番でセルに割り当てられます。
