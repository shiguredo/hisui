...
- `--features fdk-aac` を有効にしてビルドし、`--fdk-aac` オプションまたは `HISUI_FDK_AAC_PATH` 環境変数で FDK-AAC の共有ライブラリが指定された場合

FDK-AAC を利用する場合は、[ビルド方法](build.md) を参考にして、共有ライブラリのパスを指定してください。
